...
def test_cpp_compilation(cppFile):
  ...
  """
    ...
    return os.system(f"g++ {cppFile} -o {exeFile} &> /dev/null") == 0
...
def iterate_for_compilable_solution(prompt, maxIterations):
    ...
    for it in range(maxIterations):
        ...
        with contextlib.redirect_stdout(None):
            isSolutionCompilable = test_cpp_compilation(f"{fileName}.cpp")
        if isSolutionCompilable:
            print(f"Found a compilable solution after {it + 1} iterations")
            print(f"C++ File: {fileName}.cpp")
            print(f"Compiled Executable: {fileName}.exe")
            break
        if it == maxIterations - 1:
            print(
                f'Unfortunately CODEX did not find a compilable solution. Still you can find the generated code in the file: {fileName}.cpp'
            )
